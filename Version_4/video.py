...
def sauvegarder_diagramme(particules_crees, particules_capturees, particules_transmises) :
    pourcentage = np.zeros(15)
    for i in range(len(particules_crees)) :
        if particules_crees[i] == 0 :
            pourcentage[i] = 100
        else :
            pourcentage[i] = 100 - 100 * particules_transmises[i] / particules_crees[i] #100 * particules_capturees[i] / particules_crees[i]
    plt.clf()
